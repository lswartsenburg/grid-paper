...
describe('shapeBounds', () => {
  ...
  it('computes bounds for a single line', () => {
    const line: VectorShape = { ...BASE, id: 'l1', type: 'line', points: [{ x: 1, y: 2 }, { x: 5, y: 8 }] };
    expect(shapeBounds([line])).toEqual({ minX: 1, minY: 2, maxX: 5, maxY: 8 });
  });

  it('computes bounds for a circle', () => {
    const circle: VectorShape = { ...BASE, id: 'c1', type: 'circle', center: { x: 10, y: 10 }, radius: 3, fillColor: 'transparent' };
    expect(shapeBounds([circle])).toEqual({ minX: 7, minY: 7, maxX: 13, maxY: 13 });
  });

  it('computes bounds for a rect', () => {
    const rect: VectorShape = { ...BASE, id: 'r1', type: 'rect', origin: { x: 2, y: 3 }, width: 6, height: 4, fillColor: 'transparent' };
    expect(shapeBounds([rect])).toEqual({ minX: 2, minY: 3, maxX: 8, maxY: 7 });
  });

  it('merges bounds of multiple shapes', () => {
    const a: VectorShape = { ...BASE, id: 'a', type: 'line', points: [{ x: 0, y: 0 }, { x: 2, y: 2 }] };
    const b: VectorShape = { ...BASE, id: 'b', type: 'line', points: [{ x: -1, y: 3 }, { x: 5, y: 1 }] };
    expect(shapeBounds([a, b])).toEqual({ minX: -1, minY: 0, maxX: 5, maxY: 3 });
  });
});
...
describe('hitTestDocument — misses', () => {
  ...
  it('returns null when clicking on a locked layer', () => {
    const shape: VectorShape = { ...BASE, id: 's1', type: 'line', points: [{ x: 0, y: 0 }, { x: 10, y: 0 }] };
  ...
  it('misses a line when farther than 8 screen pixels away', () => {
    ...
    const line: VectorShape = { ...BASE, id: 'l1', type: 'line', points: [{ x: 0, y: 0 }, { x: 10, y: 0 }] };
    // Click slightly beyond the threshold
    const beyondThreshold = PX8_IN_GRID + 0.01; // just over 8px in grid units
    expect(hitTestDocument(makeDoc(line), { x: 5, y: beyondThreshold }, 1)).toBeNull();
  });

  it('misses the interior of a transparent-fill rect', () => {
    const rect: VectorShape = { ...BASE, id: 'r1', type: 'rect', origin: { x: 0, y: 0 }, width: 10, height: 10, fillColor: 'transparent' };
...
describe('hitTestDocument — hits', () => {
  ...
  it('hits a line within 8 screen pixels', () => {
    ...
    const result = hitTestDocument(makeDoc(line), { x: 5, y: withinThreshold }, 1);
    expect(result).toEqual({ layerId: 'layer-1', shapeId: 'l1' });
  });
  ...
  it('hits the border of a transparent-fill rect', () => {
    ...
    const result = hitTestDocument(makeDoc(rect), { x: 5, y: 1 / BASE_UNIT }, 1);
    expect(result).toEqual({ layerId: 'layer-1', shapeId: 'r1' });
  });

  it('hits the interior of a filled rect', () => {
    const rect: VectorShape = { ...BASE, id: 'r1', type: 'rect', origin: { x: 0, y: 0 }, width: 10, height: 10, fillColor: '#ff0000' };
    const result = hitTestDocument(makeDoc(rect), { x: 5, y: 5 }, 1);
    expect(result).toEqual({ layerId: 'layer-1', shapeId: 'r1' });
  });

  it('hits a circle on its circumference', () => {
    const circle: VectorShape = { ...BASE, id: 'c1', type: 'circle', center: { x: 5, y: 5 }, radius: 3, fillColor: 'transparent' };
  ...
  it('hits a filled circle anywhere inside', () => {
    const circle: VectorShape = { ...BASE, id: 'c1', type: 'circle', center: { x: 5, y: 5 }, radius: 3, fillColor: '#blue' };
...
describe('hitTestDocument — threshold is 8 screen pixels', () => {
  it('does NOT hit a shape 10 screen pixels away (old buggy threshold would have)', () => {
    ...
    expect(hitTestDocument(makeDoc(line), { x: 5, y: tenScreenPx }, 1)).toBeNull();
  });

  it('DOES hit a shape exactly 8 screen pixels away', () => {
    const line: VectorShape = { ...BASE, id: 'l1', type: 'line', points: [{ x: 0, y: 0 }, { x: 10, y: 0 }] };
    const eightScreenPx = 8 / BASE_UNIT; // 0.4 grid units — exactly at threshold
    expect(hitTestDocument(makeDoc(line), { x: 5, y: eightScreenPx }, 1)).toEqual({
      layerId: 'layer-1',
      shapeId: 'l1',
    });
  });
  ...
  it('hit radius scales correctly with zoom — at zoom=2, threshold is still 8px', () => {
    ...
    expect(hitTestDocument(makeDoc(line), { x: 5, y: eightScreenPxAtZoom2 }, 2)).toEqual({
      layerId: 'layer-1',
      shapeId: 'l1',
    });
    // And 10px away should miss
    const tenScreenPxAtZoom2 = 10 / (BASE_UNIT * 2);
    expect(hitTestDocument(makeDoc(line), { x: 5, y: tenScreenPxAtZoom2 }, 2)).toBeNull();
  });
});
...
describe('hitTestDocument — paint order', () => {
  it('returns the topmost (last painted) shape when two shapes overlap', () => {
    const bottom: VectorShape = { ...BASE, id: 'bottom', type: 'line', points: [{ x: 0, y: 0 }, { x: 10, y: 0 }] };
    const top: VectorShape = { ...BASE, id: 'top', type: 'line', points: [{ x: 0, y: 0 }, { x: 10, y: 0 }] };
